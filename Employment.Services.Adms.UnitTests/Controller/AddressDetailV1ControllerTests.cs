using Employment.Services.ReferenceData.WebApi.Controllers;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using Employment.Infrastructure.Security.Contract;
using Employment.Services.Infrastructure.UnitTesting;
using System.Diagnostics.CodeAnalysis;

namespace Employment.Services.ReferenceData.UnitTests.Controller
{
    [TestClass]
      [ExcludeFromCodeCoverage]
  public class AddressDetailV1ControllerTests: ControllerUnitTestBase<Startup>
    {

        /// <summary>
        /// Create a dummy list of AuthorisationClaimMappings. The mock authoriser will use these
        /// to validate security on the API calls.
        /// </summary>
        /// <returns>A list of AuthorisationClaimMappings.</returns>
        public override List<AuthorisationClaimMapping> GetAuthorisationClaimMappings()
        {
            // create authoriser mappings that are likely to be hit by the current tests
            var mappings = new List<AuthorisationClaimMapping>();
            mappings.Add(new AuthorisationClaimMapping() {ActivityName = "AAA-YourActivityName",ClaimSchema = "http://deewr.gov.au/es/2011/03/claims/baserole", ClaimValue = "DAD"});
            return mappings;
        }

        private AddressDetailV1Controller CreateAddressDetailV1Controller()
        {
#if NETFRAMEWORK
            return new AddressDetailV1Controller();
#else
            return new AddressDetailV1Controller(null);
#endif
        }

        [TestMethod]
        public async Task AddressDetail_StateUnderTest_ExpectedBehavior()
        {
            // Arrange
            var addressDetailV1Controller = this.CreateAddressDetailV1Controller();
            int addressId = 0;
            string boundarySpecification = null;
            string formatSpecifier = null;
            string context = null;

            // Act
            var result = await addressDetailV1Controller.AddressDetail(
                addressId,
                boundarySpecification,
                formatSpecifier,
                context);

            // Assert
            Assert.Fail();
        }

        [TestMethod]
        public async Task AddressDetail_StateUnderTest_ExpectedBehavior1()
        {
            // Arrange
            var addressDetailV1Controller = this.CreateAddressDetailV1Controller();
            string formattedAddress = null;
            string boundarySpecification = null;
            string formatSpecifier = null;
            string context = null;
            string locality = null;
            string postCode = null;
            string state = null;
            int addressId = 0;

            #if NETFRAMEWORK
            var result = await addressDetailV1Controller.AddressDetail(
                formattedAddress,
                boundarySpecification,
                formatSpecifier,
                context,
                locality,
                postCode,
                state);
#else
            var result = await addressDetailV1Controller.AddressDetail(
                addressId,
                formattedAddress,
                boundarySpecification,
                formatSpecifier,
                context,
                locality,
                postCode,
                state);
            #endif

            // Assert
            Assert.Fail();
        }
    }
}
