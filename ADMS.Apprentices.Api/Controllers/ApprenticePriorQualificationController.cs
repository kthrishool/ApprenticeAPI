...
namespace ADMS.Apprentices.Api.Controllers
{
    ...
    /// </summary>
    [ApiController]
    [Route("v1/apprentices/{apprenticeId}/prior-qualifications")]
    [Route("apprentices/{apprenticeId}/prior-qualifications")]
    ...
    public class ApprenticePriorQualificationController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<PriorQualificationModel[]>> List(int apprenticeId)
        {
            Profile profile = await repository.GetAsync<Profile>(apprenticeId, true);
            PriorQualificationModel[] models = profile.PriorQualifications.Map(a => new PriorQualificationModel(a)).ToArray();
            return Ok(models);
        }
        ...
        [Authorize(Policy = AuthorisationConfiguration.AUTH_Apprentice_View)]
        [HttpGet("{id}")]
        public async Task<ActionResult<PriorQualificationModel>> Get(int apprenticeId, int id)
        {
            Profile profile = await repository.GetAsync<Profile>(apprenticeId, true);
            PriorQualification qualification = profile.PriorQualifications.Get(q => q.Id, id);
            return Ok(new PriorQualificationModel(qualification));
        }
        ...
        [HttpPost]
        public async Task<ActionResult<PriorQualificationModel>> Create(int apprenticeId, [FromBody] PriorQualificationMessage message)
        {
            PriorQualification qualification = await qualificationCreator.CreateAsync(apprenticeId, message);
            await repository.SaveAsync();
            return Created($"/{qualification.Id}", new PriorQualificationModel(qualification));
        }
        ...
        [Authorize(Policy = AuthorisationConfiguration.AUTH_Apprentice_Management)]
        [HttpPut("{id}")]
        public async Task<ActionResult<PriorQualificationModel>> Update(int apprenticeId, int id, [FromBody] PriorQualificationMessage message)
        {
